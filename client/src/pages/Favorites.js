                ...
                API.getGameStats(this.state.favGames[i])
                  .then(statsRes => {
                    ...
                    let gameObj = {
                      ...
                      gameTime: Moment.utc(
                        gameRes.data.api.games[0].startTimeUTC
                      )
                        .utcOffset(-8)
                    ...
                    return !event.finished ? (
                      <Col size="md-6 sm-6">
                        <EventCard
                          ...
                          gameTime={Moment.utc(event.game.games[0].startTimeUTC)
                            .utcOffset(-8)
                            .format("dddd, MMMM Do YYYY, h:mm a")}
                          onClick={() =>
                            this.handleSubmit(
                              event.game.games[0].gameId,
                              this.state.favGames.includes(
                                event.game.games[0].gameId
                              )
                            )
                          }
                          favorited={this.state.favGames.includes(
                            event.game.games[0].gameId
                          )}
                          awayTeamLogo={event.game.games[0].vTeam.logo}
                          homeTeamLogo={event.game.games[0].hTeam.logo}
                          calendarClick={() =>
                            this.addGametoCalender(event.game.games[0])
                          }
                        ></EventCard>
                      </Col>
                    ) : (
                      <Col size="md-12">
                        <GameStatsCard
                          ...
                          deleteButton={true}
                          onClick={() =>
                            this.handleSubmit(event.stats[0].gameId, true)
                          }
                        ></GameStatsCard>
                      </Col>
                    );
